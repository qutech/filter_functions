...
the number of time-dependent control operators (if identifiers are provided,
...
:func:`liouville_derivative`
    Calculate the derivatives of the control propagators in Liouville space.
...
def liouville_derivative(
    ...
    c_coeffs : array_like, shape (n_cops, n_dt)
        The control strengths of the control operators given by *c_opers* for
        each time step.
    ...
    HD : array_like, shape (n_dt, d)
        ...
        axis counting the pulse segment, i.e.
        ``HD == array([D_0, D_1, ...])``.
    VHV : array_like, shape (n_dt, c_ctrl, d, d)
        The control operators transformed into the eigenspace of the control
        Hamiltonian. The drift operators are ignored, if identifiers for time-
        dependent control operators are provided.
    ...
    partial_U = np.empty((n, VHV.shape[1], d, d), dtype=complex)
    deriv_U = np.zeros((n, n, VHV.shape[1], d, d), dtype=complex)

    # intermediate = Q[1:] @ Q[:-1].conj().T @ HV[:-1]
    for g in (range(n)):
        omega_diff = np.subtract.outer(HD[g], HD[g])
        ...
        partial_U[g] = -1j * np.einsum('ia,ja,jk,hkl,kl,ml->him', Q[g+1],
                                       Q[g].conj(), HV[g], VHV[g], A_mat,
                                       HV[g].conj(),
                                       optimize=['einsum_path', (3, 4), (0, 1),
                                                 (0, 3), (0, 1), (0, 1)])
        # partial_U[g] = -1j * np.einsum('ik,hkl,kl,ml->him',
        #                                intermediate[g],
        #                                VHV[g], A_mat, HV[g].conj(),
        #                                optimize=['einsum_path', (3, 4), (0, 1),
        #                                          (0, 3), (0, 1), (0, 1)])
        # Calculate the whole propagator derivative
        for g_prime in range(g+1):  # condition g' <= g-1
            deriv_U[g, g_prime] = np.einsum('ij,kj,hkl,lm->him', Q[g+1],
                                            Q[g_prime+1].conj(),
                                            partial_U[g_prime], Q[g_prime],
                                            optimize=['einsum_path', (0, 1),
                                                      (0, 1), (0, 1)])
            # deriv_U[g, g_prime] = (Q[g+1] @ Q[g_prime+1].conj().T @
            #                        partial_U[g_prime] @ Q[g_prime])

    # Now calculate derivative of Liouville representation
    # Calculate traces first to save memory
    sum1 = np.einsum('tshba,jbc,tcd,kda->thsjk', deriv_U.conj(), basis, Q[1:],
                     basis, optimize=['einsum_path', (1, 2), (0, 2), (0, 1)])
    sum2 = np.einsum('tba,jbc,tshcd,kda->thsjk', Q[1:].conj(), basis, deriv_U,
...
def control_matrix_at_timestep_derivative(
    ...
    VHV : array_like, shape (n_dt, n_ctrl, d, d)
        The control operators transformed into the eigenspace of the control
        Hamiltonian. The drift operators are ignored, if identifiers for time-
        dependent control operators are provided.
    ...
    .. math::
        ...
        \left( \bar{B}_{\alpha} \cdot \mathds{M} \right).
    ...
    .. math::
        ...
        \mathds{M}_{mn} = \left[ \bar{C}_j, \mathds{I}^{(mn)}
    ...
    .. math::
        ...
        \mathds{I}_{pq}^{(mn)} &= \delta_{pq} \left(
            \frac{\Delta t_g \cdot
    ...
    VBV = np.einsum('gji,ajk,gkl->gail', HV.conj(), n_opers, HV, optimize=path)
    VCV = np.einsum('tnm,jnk,tkl->tjml', HV.conj(), basis, HV, optimize=path)
    ...
    I_circ_H = np.einsum('toijnm,thij->tohijnm', integral_deriv, VHV)
...
def calculate_derivative_of_control_matrix_from_scratch(
    ...
    c_coeffs: Sequence[Coefficients],
    all_id: Sequence[str],
    c_id: Optional[Sequence[str]] = None) -> ndarray:
    r"""
    Calculate the derivative of the control matrix from scratch.
    ...
    c_coeffs : array_like, shape (n_cops, n_dt)
        The control strengths of the control operators given by *c_opers* for
        each time step.
    all_id : array_like, shape (n_cops)
        Identifiers of all control operators.
    c_id : Sequence[str], shape (n_ctrl), Optional
        Sequence of strings with the control identifiern to distinguish between
        control and drift Hamiltonian. The default is None.

    Raises
    ------
    ValueError
        If the given identifiers *c_id* are not subset of the total identifiers
        *all_id* of all control operators.
    ...
    path = ['einsum_path', (0, 1), (0, 1)]
    if (c_id is None):
        VHV = np.einsum('tji,hjk,tkl->thil', HV.conj(), c_opers, HV,
                        optimize=path)
    elif (set(c_id) <= set(all_id)):
        dict_id_oper = dict(zip(all_id, c_opers))
        control = [dict_id_oper[element] for element in c_id]
        VHV = np.einsum('tji,hjk,tkl->thil', HV.conj(), control, HV,
                        optimize=path)
    else:
    ...
    dL = liouville_derivative(t, dt, Q, c_coeffs, c_opers, basis, HV, HD, VHV)
    ctrlmat_data = control_matrix_at_timestep_derivative(
        omega, dt, HD, HV, basis, c_opers, c_coeffs, n_opers, n_coeffs, VHV)
...
def calculate_canonical_filter_function_derivative(
    ...
    Returns
    -------
    deriv_F : ndarray, shape (n_nops, n_dt, n_ctrl, n_omega)
        The regular filter functions' derivatives for variation in each control
        contribution :math:`\frac{\partial F_\alpha}{\partial u_h(t_{g'})}`.
...
def infidelity_derivative(
    ...
    *pulse*.
    ...
    c_id : Sequence[str], shape (n_ctrl)
        Sequence of strings with the control identifiern to distinguish between
        control and drift Hamiltonian.
    ...
    deriv_F = pulse.get_filter_function_derivative(omega, c_id)
    d = pulse.d
