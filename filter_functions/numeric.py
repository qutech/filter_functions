...
def calculate_control_matrix_from_atomic(
    ...
    # Allocate memory
    if which == 'total':
        control_matrix = np.zeros(control_matrix_atomic.shape[1:],
                                  dtype=complex)
        for g in util.progressbar_range(n, show_progressbar=show_progressbar,
                                        desc='Calculating control matrix'):
            control_matrix += expr(phases[g]*control_matrix_atomic[g],
        ...
        for g in util.progressbar_range(n, show_progressbar=show_progressbar,
                                        desc='Calculating control matrix'):
            control_matrix[g] = expr(phases[g]*control_matrix_atomic[g],
                                     propagators_liouville[g])
...
def calculate_control_matrix_from_scratch(
    ...
    if d < 4:
        ...
        n_opers_transformed = np.empty((len(n_opers), len(dt), d, d),
                                       dtype=complex)
        for j, n_oper in enumerate(n_opers):
            n_opers_transformed[j] = (eigvecs.conj().transpose(0, 2, 1)
                                      @ n_oper
                                      @ eigvecs)

    # Allocate result and buffers for intermediate arrays
    control_matrix = np.zeros((len(n_opers), len(basis), len(E)),
                              dtype=complex)
...
def calculate_control_matrix_periodic(phases: ndarray, control_matrix: ndarray,
    ...
    control matrix and transfer matrix of the total propagator,
    ...
    control_matrix_tot = (
        control_matrix.transpose(2, 0, 1) @ S
    ).transpose(1, 2, 0)
...
def calculate_pulse_correlation_filter_function(
        control_matrix: ndarray, which: str = 'fidelity') -> ndarray:
    ...
    Returns
    -------
    filter_function_pc: ndarray, shape (n_pulses, n_pulses, n_nops, n_nops, [d**2, d**2], n_omega)  # noqa
...
def diagonalize(H: ndarray, dt: Coefficients) -> Tuple[ndarray]:
    ...
    #     P[l] = (V * D) @ V.conj().T
    P = np.einsum('lij,jl,lkj->lik', eigvecs,
                  util.cexp(-np.asarray(dt)*eigvals.T), eigvecs.conj())
...
def error_transfer_matrix(
    ...
    if d == 2 and pulse.basis.btype in ('Pauli', 'GGM'):
        ...
        # Offdiagonal terms
        error_transfer_matrix[..., ~diag_mask] = -(
        ...
        for i in range(1, N):
            error_transfer_matrix[..., i, i] = \
                u_kl[..., diag_items, diag_items].sum(axis=-1)
        ...
        error_transfer_matrix = (
            oe.contract('...kl,klij->...ij', u_kl, T, backend='sparse')/2 +
...
def infidelity(pulse: 'PulseSequence',
    ...
    test_convergence: bool, optional
        ...
        corresponding fidelities. See *spectrum* and *omega* for more
        information.

    Returns
    -------
    infid: ndarray
        Array with the infidelity contributions for each spectrum *spectrum* on
        the last axis or axes, depending on the shape of *spectrum* and
        *which*. If ``which`` is ``correlations``, the first two axes are the
        individual pulse contributions. If *spectrum* is 2-d (3-d), the last
        axis (two axes) are the individual spectral contributions.
    ...
    if test_convergence:
        if not callable(spectrum):
            raise TypeError('Spectrum should be callable when ' +
        ...
        for i, n in enumerate(n_samples):
            ...
            convergence_infids[i] = infidelity(
                pulse, *util.symmetrize_spectrum(spectrum(freqs), freqs),
    ...
    if which == 'total':
        if not pulse.basis.istraceless:
            ...
            filter_function = np.einsum('ako,blo,kl->abo',
                                        control_matrix.conj(), control_matrix,
                                        Tp)/pulse.d
        else:
...
def liouville_representation(U: ndarray, basis: Basis) -> ndarray:
    ...
    if basis.btype == 'GGM' and basis.d > 12:
        ...
        control_matrix = ggm_expand(conjugated_basis).real
    else:
        path = ['einsum_path', (0, 1), (0, 1), (0, 1)]
        control_matrix = np.einsum('...ba,ibc,...cd,jda',
                                   U.conj(), basis, U, basis,
                                   optimize=path).real
...
def _get_integrand(
        spectrum: ndarray, omega: ndarray, idx: ndarray,
    ...
    if control_matrix is not None:
        # left is the complex conjugate
        funs = (np.conj, lambda x: x)
        if isinstance(control_matrix, (list, tuple)):
            left, right = [f(c) for f, c in zip(funs, control_matrix)]
        else:
            left, right = [f(c) for f, c in zip(funs, [control_matrix]*2)]
    ...
    if spectrum.ndim == 1:
        ...
        if filter_function is not None:
            ...
            integrand = np.einsum('jko,jlo->jklo', left, spectrum*right).real
        ...
        if filter_function is not None:
            ...
            integrand = np.einsum('jko,jo,jlo->jklo',
                                  left, spectrum, right).real
        ...
        if filter_function is not None:
            ...
            integrand = np.einsum('iko,ijo,jlo->ijklo', left, spectrum, right)
    elif spectrum.ndim > 3:
